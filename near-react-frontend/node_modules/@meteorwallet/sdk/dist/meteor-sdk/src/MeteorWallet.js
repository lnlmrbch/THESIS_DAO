...
class MeteorWallet {
     ...
     * @hidden
     *
     * Requests a wallet sign-in, using the website redirect method.
     * Redirects current page to the wallet authentication page.
     *
     * @example
     * ```js
     * const wallet = new MeteorWallet({ near: connectedNear, appKeyPrefix: 'my-app' });
     * // redirects to Meteor Wallet
     * wallet.requestSignIn({ contractId: 'account-with-deploy-contract.near' });
     * ```
     */
    /*async requestSignIn_redirect(
      options: PartialBy<TMeteorWalletExternalAction_SignIn_Data, "public_key"> & Partial<IWithCallbackUrl>,
    ) {
      const currentUrl = new URL(window.location.href);
      const newUrl = new URL(this._walletBaseUrl + LOGIN_WALLET_URL_SUFFIX);
      newUrl.searchParams.set("callback_url", options.callback_url ?? currentUrl.href);
      const contractAccount = await this._near.account(options.contract_id);
      await contractAccount.state();
      newUrl.searchParams.set("contract_id", options.contract_id);
      const accessKey = KeyPair.fromRandom("ed25519");
      newUrl.searchParams.set("public_key", accessKey.getPublicKey().toString());
      newUrl.searchParams.set("network", this._networkId);
      await this._keyStore.setKey(this._networkId, PENDING_ACCESS_KEY_PREFIX + accessKey.getPublicKey(), accessKey);
  
      if (options.type === EMeteorWalletSignInType.SELECTED_METHODS) {
        newUrl.searchParams.set("type", EMeteorWalletSignInType.SELECTED_METHODS);
        for (const method of (options as TMeteorWalletExternalAction_SignIn_SelectedMethods).methods) {
          newUrl.searchParams.append("methods", method);
        }
      } else {
        newUrl.searchParams.set("type", EMeteorWalletSignInType.ALL_METHODS);
      }
    ...
    async requestSignIn(options) {
        ...
        /*if (this.isExtensionInstalled()) {
          return window.meteorWallet.requestSignIn(restOptions);
        } else {
     ...
     * @hidden
     *
     * Requests the user to quickly sign for a transaction or batch of transactions by redirecting to Meteor wallet.
     */
    /*async requestSignTransactions_redirect({
      transactions,
      meta,
      callback_url,
    }: IORequestSignTransactionsRedirect_Inputs): Promise<void> {
      const currentUrl = new URL(window.location.href);
      const newUrl = new URL(this._walletBaseUrl + SIGN_WALLET_URL_SUFFIX);
  
      console.log("Request Sign Transactions (REDIRECT)", { transactions, meta, callback_url });
      ...
      newUrl.searchParams.set("callback_url", callback_url ?? currentUrl.href);
    ...
    async requestSignTransactions(inputs) {
        ...
        /*if (this.isExtensionInstalled()) {
          return window.meteorWallet.requestSignTransactions({ transactions });
        } else {
